...
################# loss functions for infrared and visible image fusion #################

@LOSS_REGISTRY.register()
class SSIMFusionLoss(nn.Module):
    def __init__(self, window_size=11, size_average=True, loss_weight=1.0, reduction='mean'):
        super(SSIMFusionLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, image_vis, image_ir, generate_img):
        ...
        return self.loss_weight * (1-self.ssim(generate_img_y, image_y, self.window_size, self.size_average)) + self.loss_weight * (1-self.ssim(generate_img_y, image_ir, self.window_size, self.size_average))
